export default {
  tab1Render: {
    theme_choice: "Karten-Design wählen:",
    light: "Hell",
    dark: "Dunkel",
    auto: "Auto",
    devices_per_column: "Anzahl der \"Geräte\" pro Spalte:",
	max_heigth: "Maximale Boxhöhe (%):",
    font_size_zones: "Schriftgröße in \"Geräte\"-Zonen:",
    in_header: "in Kopfzeile",
    in_devices: "in Geräten",
    in_footer: "in Fußzeile",
    font_size: "Schriftgröße"
  },
  subtabRender: {
    header_title: "Kopfzeilen-Konfiguration",
    icon_choice: "Symbol-Auswahl",
    name_choice: "Gerätename",
    box_height: "Boxhöhe, % der Spalte (optional)",
    list_title: "Infoliste",
    enable_list: "Als Infoliste anzeigen",
    list_add_row: "Zeile hinzufügen",
    list_row_name: "Name",
    sensor_title: "Hauptsensor-Konfiguration",
    entity_choice: "Entitäts-Auswahl",
    entity2_choice: "Zweite Entitäts-Auswahl",
    entity2_label: "Bezeichnung der zweiten Entität (optional)",
    enable_graph: "Grafik aktivieren",
	graph_range: "Diagrammdauer (Std.)",
	enable_graphMinMax: "Min. / Max. anzeigen",
    enable_gauge: "Anzeige in % aktivieren",
	sensorSignIcon: "Richtungssymbol anzeigen",
	gauge_pct_title: "Konfiguration der Hauptanzeige",
    warning_gauge: "Kann nur aktiviert werden, wenn die Maßeinheit % ist",
	gauge_wave_entity: "Entität auswählen",
	enable_gaugeTexture: "Textur anzeigen",
	header_actions_title: "Aktionen",
	header_tap_actions: "Aktion \"Tap\"",
	side_gauge_title: "Konfiguration der seitlichen Anzeige",
	side_gauge_entity: "Entität auswählen",
	side_gauge_max: "Maximalwert",
    header_footer_title: "Kopf- und Fußzeilen-Sensoren Konfiguration",
    entity_header: "Kopfzeilen-Entität wählen",
    header_label: "Kopfzeilen-Bezeichnung (optional)",
    entity_footer: "Fußzeilen-Entität wählen",
    footer_label: "Fußzeilen-Bezeichnung (optional)",
    entity2_footer: "2. Fußzeilen-Entität wählen",
    footer2_label: "2. Fußzeilen-Bezeichnung (optional)",
    entity3_footer: "3. Fußzeilen-Entität wählen",
    footer3_label: "3. Fußzeilen-Bezeichnung (optional)",
    anchor_title: "Anker hinzufügen",
    left_qtyBox: "Links",
    top_qtyBox: "Oben",
    bottom_qtyBox: "Unten",
    right_qtyBox: "Rechts",
    add_links: "Links hinzufügen",
    add_link: "Link hinzufügen"
  },
  addLink: {
    start: "Start",
    end: "Ende",
    entity_picker: "Entität wählen",
    reverse: "Umkehren",
	threshold: "Türschwelle"
  },
  typeAction: {
    def: "Standard (mehr Infos)",
    info: "Mehr Infos",
    nav: "Navigation",
    tgl: "Umschalten",
    srv: "Dienst aufrufen",
    none: "Keine"
  },
  actionArea: {
    def_entity: "Entität (optional)",
    nav_path: "Navigationspfad",
    service: "Dienst (domain.service)",
    service_entity: "Entitäts-ID",
    service_value: "Wert (optional)"
  }
};
